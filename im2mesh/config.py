...
def get_dataset(mode, cfg, return_idx=False, return_category=False):
    ...
    rotation_folder = cfg['data']['rotation_path']
    rotation_augment = cfg['data']['rotation_augment']
    ...
    if dataset_type == 'Shapes3D':
        ...
        transform = None
        if rotation_augment == 'pca':
            transform = data.transforms.PointcloudPCA()
        if mode == 'train' and rotation_augment == 'so3_rand':
            transform = data.transforms.PointcloudRandomRotate()
        dataset = data.Shapes3dDataset(
            dataset_folder, fields,
            split=split,
            categories=categories,
            rotation_folder=rotation_folder,
